// Isaac Sim bridge for the Digital Twin Robotics (Unitree H1) module.
...
const HUMANOID_EVENT_TYPES = new Set([
  "h1_train_metric", "h1_deploy_phase", "h1_teleop_step", "h1_log", "h1_snapshot", "complete", "failed"
]);
...
export function createIsaacBridge(config) {
  ...
  function registerExecutor(info = {}) {
    executor = { id: text(info.id || "isaac-executor", 64), name: text(info.name || "Isaac Sim", 80), scene: text(info.scene, 120), version: text(info.version, 40), dryRun: Boolean(info.dryRun), lastSeen: now() };
    return executor;
  }
  ...
  function descriptor() {
    ...
    return {
      ...
      connected: connected(),
      executor: executor ? { id: executor.id, name: executor.name, scene: executor.scene, version: executor.version, dryRun: executor.dryRun, lastSeenMs: now() - executor.lastSeen } : null,
  ...
  function nextMission(info) {
    ...
    wake(job);
    return { humanoidJob: publicHumanoidJob(job) };
  }
  ...
  function pushEvents(jobId, events) {
    ...
    let accepted = 0;
    for (const raw of events.slice(0, 50)) { const event = normalizeHumanoid(raw); if (event) { job.queue.push(event); accepted += 1; } }
  ...
  return { authorize, descriptor, nextMission, pushEvents, runHumanoidJob, connected };
}
